...
for checkbox in checkboxes:
    ...
    if label.text in items_to_select: # 선택 항목과 일치시
        checkbox.click()
...
for idx in range(1,40): # 1이상 40 미만 페이지 반복
    # 사전 작업 : 페이지 이동 
    browser.get(url+str(idx)) # http ..&page =2 

    #5. 데이터 추출 
    df= pd.read_html(browser.page_source)[1]
    df.dropna(axis='index',how='all',inplace=True) # 결측지 제거 
    df.dropna(axis='columns',how='all',inplace=True)
    if len(df)==0:  # 더이상 가져올 데이터 없으면 
        break
#6. 파일 저장 
    f_name='sisex.csv'
